...
def monkeypatch_request_folder(runestone_env, tmp_path):
    ...
    current = runestone_env["current"]
    current.get_course_row = mock_get_course_row
    # Pass the check in ``team.team_report`` that the current user is an instructor.
    current.verifyInstructorStatus = lambda *args: True
    auth = _object()
    auth.user = _object()
...
def get_team_members(
    runestone_env, runestone_name, monkeypatch_request_folder, monkeypatch
):
    return module_import(
        monkeypatch, runestone_env, runestone_name, "team", "get_team_members"
    )


# A fixtured version of importing ``team_report``.
@pytest.fixture
def team_report(runestone_env, runestone_name, monkeypatch_request_folder, monkeypatch):
    return module_import(
        monkeypatch, runestone_env, runestone_name, "team", "team_report"
    )


# Emulate ``from package import module`` when run in a Runestone controller.
def module_import(monkeypatch, runestone_env, runestone_name, package, module):
    monkeypatch.syspath_prepend(
        "applications/{}/modules".format(
            runestone_env["request"].application, runestone_name
        )
    )
    exec("from {} import {}".format(package, module), runestone_env)
    return runestone_env[module]
...
def test_3(get_team_members_from_csv):
    team_name, team_member_list = get_team_members_from_csv(u"\n", "")
    assert team_name.startswith("Error: User ID  not in list of team members.")


# Test wrong headings.
def test_4(get_team_members_from_csv):
    team_name, team_member_list = get_team_members_from_csv(
        u"user id,user name,team names\n", ""
    )
...
def test_9(get_team_members_from_csv):
    ...
    team_name, team_member_list = get_team_members_from_csv(
        csv_contents, "user_id3", "course_name2"
    )
...
def test_team_report_1(make_csv, team_report):
    ...
    eval_data_dict, team_data_dict, grades = team_report(
        TEST_SUBCHAPTER, TEST_COURSE_NAME
    )
    ...
    assert team_data_dict["team_name2"].team_netids == [
        "user_id1",
        "user_id3",
        "user_id2",
    ]
...
def test_team_report_2(make_csv, team_report):
    make_csv(u"user id,user name,team name\n")
    eval_data_dict, team_data_dict, grades = team_report(
        TEST_SUBCHAPTER, TEST_COURSE_NAME
    )
